...
export async function handleSubmitProduct(e: React.FormEvent<HTMLFormElement>, product: ProductPayload, setProduct: React.Dispatch<SetStateAction<ProductPayload>>) {
  e.preventDefault();
...
export async function getSellerProducts(setState: React.Dispatch<SetStateAction<Product[]>>): Promise<void> {
  try {
    const res = await axios.get(`http://localhost:8000/product/seller_products`, {
      withCredentials: true,
    });
...
export async function getSeller(id: string, setState: React.Dispatch<SetStateAction<string>>) {
  try {
    console.log("ID from get seller : ", id);
    const res = await axios.get(`http://localhost:8000/users/search?id=${id}`, {
      withCredentials: true,
    });

    console.log("From get seller : ", res.data);
    setState(res.data.user.stake_addr);
  } catch (err) {
    console.error(err);
  }
}
